...
# ╔═╡ b521ee36-13b1-40f6-ae96-64d69c8897e5
begin
	using HttpCommon
	using StaticArrays
	using Plots
...
# ╔═╡ 1cc3cc03-64b8-46b4-9fa4-9663b0b251f3
md"""
# Read a Slugs »Explicit Strategy«
"""
...
"""
	get_integer_value(name::T, vars::T) where T<:AbstractString

Extract the integer value for the structuredslugs variable `name` from the set of boolean `vars`.
"""
function get_integer_value(name::S, vars::T) where 
		{S<:AbstractString, T<:AbstractString}
	
	re_first_entry = Regex("$name@(?<index>0)\\.(?<lower>\\d+)\\.(?<upper>\\d+):(?<value>[01])")
	first_entry = match(re_first_entry, vars)
	lower = first_entry[:lower]
	upper = first_entry[:upper]
	bits = first_entry[:value]
	re_other_entries = Regex("$name@(?<index>\\d+):(?<value>[01])")
	matches = eachmatch(re_other_entries, vars)
	matches = sort(collect(matches), by=(x -> x[:index]))
	for m in matches
		bits *= m[:value]
	end
	bits = reverse(bits)
...
function get_integer_values(vars::T) where T<:AbstractString
	re_names = r"(?<name>[a-zA-Z_]+)@"
	names = eachmatch(re_names, vars)
	names = map(x -> x[:name], names)
	names = names |> collect |> unique
	result = Dict{String, Int64}()
	for name in names
		result[name] = get_integer_value(name, vars)
	end
	result
end
...
function get_successors(successors::T) where T<:AbstractString
	successors == "" && return Int64[]
	Int64[parse(Int64, s) for s in split(successors, ", ")]
end

# ╔═╡ 77e8fb80-1815-47b7-b2e2-30957a927ea9
successors = "55, 56, 57, 46, 48, 58, 49, 51, 59"

# ╔═╡ c0e77bdd-2614-4fe2-b759-4aac292a7830
get_successors(successors)

# ╔═╡ 6f6a7f85-07c0-4b9f-bb19-422d71e53240
function state_count(raw_string)
	re_state = r"State (?<state>\d+) with rank (?<rank>\d+) ->"
	result = typemin(Int64)
	for m in eachmatch(re_state, raw_string)
		v = parse(Int64, m[:state])
		if v > result
			result = v
		end
		if m[:rank] != "0"
			@warn "Found state with non-zero rank. I have no idea what that means. $m"
		end
	end
	result
end
...
struct ExplicitStrategy
	var_indexed::Array{State}
	state_id_indexed::Array{State}
end

# ╔═╡ eb6da291-13ff-4e00-89d7-b9101cf9e1dd
function read_strategy(raw_string)
	result = ExplicitStrategy(Array{State}(undef, 201, 31, 31), 
		Array{State}(undef, state_count(raw_string) + 1))
	...
	for i in 1:2:length(lines)
		lines[i] == "" && break
		m = match(re_successors, lines[i + 1])
		isnothing(m) && @error lines[i + 1] i
		successors = get_successors(m[:successors])
		n = match(re_state, lines[i])
		id = parse(Int64, n[:state])
		values = get_integer_values(n[:vars])
		state = State(id, values["distance"], values["v_ego"], values["v_front"], successors)
		result.state_id_indexed[id + 1] = state
		
		ix = (values["distance"] + 1, values["v_ego"] + 1, values["v_front"] + 1)
		result.var_indexed[ix...] = state
	end
	result
end

# ╔═╡ 8198654c-caaa-4d00-a305-422fb9fec122
explicit_strategy = read_strategy(raw_string)
...
function all_successors(explicit_strategy::ExplicitStrategy, state::State)
	[explicit_strategy.state_id_indexed[id + 1] for id in state.successors]
end

# ╔═╡ e7169e54-eb59-40e6-9788-2b9d4c55fd29
all_successors(explicit_strategy, state)

# ╔═╡ 7f5bbe06-3f62-403b-9877-f8662a6ec177
function random_trace(explicit_strategy::ExplicitStrategy, start_state::State; duration=100)
	distances, v_egos, v_fronts = [], [], []
	state = start_state
	for i in 0:duration
		distances ← state.distance
		v_egos ← state.v_ego
		v_fronts ← state.v_front
		successors = all_successors(explicit_strategy, state)
		length(successors) == 0 && break
		state = rand(successors)
	end
	(;distances, v_egos, v_fronts)
end

# ╔═╡ c15d6d20-627b-492d-930c-7226df9f8e4d
(distances, v_egos, v_fronts) = random_trace(explicit_strategy, state)
