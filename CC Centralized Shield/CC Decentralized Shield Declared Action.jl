...
md"""
# CC Shield with Declared Action

This is a variariant of the CC shield in which the front cars broadcasts its intended action, similar to how agents would have to negotiate their joint action under a centralized shield.

The assumption is that each period, the front car picks first, then the second car from there, and so on. This should allow a decentralized shield to achieve comparable performance to the centralized case, which outperforms a regular decentralized shield.
"""
...
# ╔═╡ e94409c5-af04-405e-b78d-91b032c9a9f3
...
[random_front_behaviour(m, rand(Uniform(0, 1))) for _ in 1:10] |> unique |> sort
...
begin
	...
	function simulate_point(mechanics::CCMechanics, 
	    ...
	    v_ego, v_front, distance, front_action = point
		front_action = CCAction(Int(front_action))
...
let
	animation = @animate for i in 1:10
		trace = simulate_sequence(m, 120, (0., 0., 50., Int(neutral)), (_...) -> rand([backwards neutral forwards]))
...
simulation_function(p, a, r) = begin
	v_ego, v_front, distance, front_action = simulate_point(m, p, r, a)
...
outer_bounds = Bounds(
	Float64[m.v_ego_min, m.v_front_min, m.distance_min, 0],
...
begin
	granularity = [granularity_v_ego, granularity_v_front, granularity_distance, 1]
...
samples_per_axis = (spa_v_ego, spa_v_front, spa_distance, 1)
...
@bind front_action Select([backwards, neutral, forwards])
...
begin
	function draw′(grid)
		draw′(grid, v_ego, v_front, distance, front_action)
	end
	function draw′(grid, v_ego, v_front, distance, front_action)
		partition = box(grid, v_ego, v_front, distance, Int(front_action))
		slice = [partition.indices[1], :, :, partition.indices[4]]
...
function shielded_random(s)
	v_ego, v_front, distance, front_action = s
	if (v_ego, v_front, distance, Int(front_action)) ∈ shield
...
if make_shield_button > 0 let
	animation = @animate for i in 1:10
		...
		trace = simulate_sequence(m, 120, (0, 0, 20, Int(neutral)), shielded_random)
...
function evaluate(m::CCMechanics, policy; episode_length=120, traces=1000)
	...
	safety_violations = 0
	for i in 1:traces
		trace = simulate_sequence(m, episode_length, (0, 0, 20, Int(neutral)), policy)
