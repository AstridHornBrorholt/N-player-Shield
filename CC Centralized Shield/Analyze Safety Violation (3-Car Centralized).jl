...
# ╔═╡ f8b3f370-ddfe-48b4-b0c0-079ae7c3e255
replace("foo", "foo" => "bar", "bar" => "baz")
...
@bind model_file TextField(80, pwd() ⨝ "3-Car Centralized.xml")
...
# ╔═╡ 086a000b-fb7d-4277-a2d4-34a5e7743c5b
begin
...
# ╔═╡ 1495e402-5832-4326-844d-f50792542542
[v for v in traces[21][50][1:3]]
...
# ╔═╡ 44630ec2-31a6-4481-8138-0ee7bca25094
any((s[velocity0] < -4) for s in traces[1])
...
middle(s) = [s[1] + 1, s[2] + 1, s[3] + 1, s[4] + 0.5, s[5] + 0.5]

# ╔═╡ d83f5fcb-5e4f-4315-af4b-775615693337
middle(s), middle(s′)

# ╔═╡ 2ccd5645-2199-4582-8c3a-8db788a46b52
# Variable "partition" already used further up
# Adopting shorthand from paper why not
µ = box(shield, middle(s))

# ╔═╡ ef8b2419-9bd4-4c2e-a6a3-554285646882
µ′ = box(shield, middle(s′))

# ╔═╡ 323b6ae5-3d3d-4108-9580-fd1734d9b47b
Bounds(µ)

# ╔═╡ 99e9e503-bb4a-4b35-a1f2-099226cc030f
Bounds(µ′)
...
joint_action = [acceleration_to_action(a) for a in s′[7:8]]

# ╔═╡ 2ffd0d92-a701-4bf4-a78f-766103ce1950
allowed = [int_to_joint_action(i, 2) for i in int_to_actions(Int, get_value(µ))]

# ╔═╡ df8a3115-bf2c-470d-934b-06f9ac237c73
joint_action ∈ allowed

# ╔═╡ a8d28683-8c94-4b62-9e1a-15d52dbb3a9b
reachable_ix = reachability_function(µ, joint_action)

# ╔═╡ c5fcf631-5896-470e-8641-e52e917f9c91
reachable = [(GridShielding.Partition(shield, i)) for i in reachable_ix]

# ╔═╡ 2a1aabc3-cf5d-409b-9cae-2c804d5e1219
any([middle(s′) ∈ Bounds(r) for r in reachable])

# ╔═╡ 4ea987bf-2447-4719-99a9-352d37f554bb
discrepancy = !any([middle(s′) ∈ Bounds(r) for r in reachable])
...
!!! danger "Discrepancy"
	...
	At time $time, R̂ did not predict the square the state would end up in.

	State at time $time: `$s`

	Subsequent state: `$s′` 

	Predicted squares: (upper, lower)
	
	    $(prettyprint([Bounds(square) for square in reachable], "\n        "))
	""")
else
	Markdown.parse("""
!!! success "UPPAAL model in agreement with reachability function."

	At time $time, R̂ predicted the square the state would end up in.
