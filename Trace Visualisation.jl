...
# ╔═╡ 381158dc-ee7b-4a4f-bfe6-22ca748d7a86
parse_trace(" (0,50) (0.02,50.0006) (0.04,50.002) (0.06,50.00419999999999)")

# ╔═╡ cae21ffa-f4b6-4100-b915-f3df570e9674
function get_traces(output::String, keyword::String)
	result = Dict{Int64, Vector{Tuple{Float64, Float64}}}()
	re_trace = Regex("$keyword\\[(?<index>\\d+)\\]:\\n\\[0\\]:(?<values>.*)", "m")
	for m in eachmatch(re_trace, output)
		index = parse(Int64, m[:index])
		values = parse_trace(m[:values])
		result[index] = values
	end
	result
end

# ╔═╡ c8ec779a-7942-42a1-afd3-4d6c87ae09da
traces = get_traces(output, "distance")

# ╔═╡ 50ffb881-ce58-4fb7-b4ea-428323bba4ce
[(t, v) for (t, v) in traces[0] if 50 <= t <= 53]
...
function at_regular_intervals(trace::T, interval::S) where T <: AbstractVector{Tuple{Float64, Float64}} where S <: Number

	t_max = trace[end][1]
	return [ value_at_time(trace, i) for i in 0:interval:prevfloat(t_max) ]
end

# ╔═╡ 08ecead1-2830-47ad-8fc6-c5ca23c66d5a
function read_distances(output)
	result = Dict{Int64, Vector{Float64}}()
	traces = get_traces(output, "distance")
	for (k, t) in traces
		result[k] = at_regular_intervals(t, Δt)
	end
	result
end

# ╔═╡ f480a6a6-4360-4585-a6ad-660bb4dd7052
distances = read_distances(output)

# ╔═╡ fded47a0-b58e-4ff0-a7fe-d6ff39c25d59
@bind t NumberField(1:(length(distances[0]) - 1), default=10/Δt)

# ╔═╡ edbe2bdd-e5bb-4560-88a2-943fe56d5f88
value_at_time(traces[0], t)

# ╔═╡ 55fef654-1ed2-469a-ae60-37c7bd92aefe
at_regular_intervals(traces[0], Δt)[Int64(t/Δt) + 1]

# ╔═╡ 997851dd-1654-4739-97a7-77ce310ee92e
...
begin
	plot()
	for (i, d) in sort(collect(distances), by=(x -> x[1]))
		plot!(d, 
			label="car$i",
			linewidth=3)
	end
...
let
	sorted = sort(collect(distances), by=(x -> x[1]))
	distances = [ d[t + 1] for (i, d) in sorted]
end
...
manifest_format = "2.0"
project_hash = "7a75fb3def0e2d8d488c1daeaa0e0d0cbe13f3e1"
...
[[deps.Gaston]]
deps = ["ColorSchemes", "DelimitedFiles", "PrecompileTools", "Random"]
git-tree-sha1 = "55b32ca83e5749f476a21c7d6c4b10abb46bfa7b"
