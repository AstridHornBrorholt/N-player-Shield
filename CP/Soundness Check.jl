...
# ╔═╡ 96c3f3c0-7939-4ad6-a41c-1b5a88a19523
begin
...
# ╔═╡ a617bfd8-a38f-45cb-9c3b-c00b09451f81
md"""
#### Authored December 2023 by Christian Schilling

Hi Asger,

I attach the code for a soundness check for linear continuous systems.
To recall, given a grid and a "barbaric fixpoint," we want to check that
the fixpoint is sound. For that, we need to check every cell (I used
this term in the code) of the grid. For each action that we allow in
this cell, the successor set must not intersect with unsafe cells (cells
with no safe action).

The code only works for linear systems. There is some preprocessing
which only has to be executed once per action. You need to give the
dynamics as a matrix (the "A" in "x' = Ax"). You can send me the ODEs of
the model if it is unclear how to write the matrix. The preprocessing
computes a new matrix (Phi) that expresses the successor relation.

Currently the code terminates when it found the first counterexample. Of
course you could instead continue and identify all cells/actions that
reach unsafe cells, and then continue with a new iteration of the
fixpoint check. But maybe first try it out and we might be lucky and not
find any counterexample.

Of course you have to adapt the code to your code base, but I hope it is
rather straightforward to integrate.

At the end of the code there is a 2D example with a 4x4 grid.

If there are questions, just ask. I may however not reply during the day.
...
I summarize the theory here, but don't worry if you do not fully follow
(it is not so easy to explain in an e-mail). The code exploits that our
grid consists of (convex) boxes. The successor of a box B is simply Phi·B. 
Since this set is convex given that B is convex, we can implement a
relatively efficient check which cells the successor set intersects
with: first compute the box approximation (which is cheap) to get a
superset of the candidate cells. Checking box intersection with a box
grid is simple (find the cells of the corners and then interpolate).
Then, due to convexity of the boxes, it is sufficient to only check the
corner cells of the overlap (although in practice I expect that this
does not save anything because the set does not overlap with a large
number of grid cells anyway).

**(Plonked into a Pluto Notebook and modified by Asger Horn Brorholt)**
"""

# ╔═╡ 1833d740-ffa1-4f81-9d9e-add91f0c8879
md"""
## grid definition and helper functions
"""

# ╔═╡ a349d59d-1181-43ef-8a4c-253a9969580c
struct Grid{N}
...
# ╔═╡ 170af9ea-c435-41e4-ae08-f8f86e3fe1cf
...
# ╔═╡ 89f875eb-dd34-4398-8be7-84cf92e088a6
...
# ╔═╡ e1c378c9-59c0-4c53-b180-5d4b26c69c57
...
# ╔═╡ ffc00b28-5a9d-49b5-aba7-d270d81b4fd9
...
# ╔═╡ 6f37ed6a-30a7-4812-b4e9-d059df6e3917
...
# ╔═╡ 5e9fb6f9-8ed5-4da8-81a1-55b1c0c52b96
...
# ╔═╡ 906f2841-be23-404b-9b79-804cf35f3bea
md"""
## Example 
"""

# ╔═╡ 618d7bce-e599-4abc-ab78-234853c51627
md"""
### Handmade Safety Strategy
"""

# ╔═╡ b143469a-9184-4abc-9678-de93059cafeb
tt = [true, true]

# ╔═╡ 1c4ae424-ec27-46ee-95fa-29cba5dcffa0
tf = [true, false]

# ╔═╡ 1b355f21-da54-4861-8a0d-8a9bbc9b998a
ft = [false, true]

# ╔═╡ 4d33398b-1322-4459-bb95-980b6d000cde
ff = [false, false]

# ╔═╡ 6295daac-225a-4c18-8f7c-c560962516f7
mat = reshape([tt, tt, tt, ff,
               ff, tt, tt, tf,
               ft, ft, ft, ft,
               ff, ff, ff, ff], (4, 4))

# ╔═╡ 9826e074-14c2-428e-97d2-953f222b6166
grid = Grid(mat,
            [-0.1, -0.1],
            [0.05, 0.05])

# ╔═╡ 01b76145-3819-4df6-afa8-bc40b371c099
indices = _indices_of_box(Hyperrectangle([0., 0.], [0.05, 0.10]), grid)

# ╔═╡ 41df0c07-64c6-4df4-bc1f-b5d954a40642
[indices...]

# ╔═╡ 4738cd07-e593-41e0-8a9b-864d77396fcd
[_isextreme(indices[i], indices) for i in 1:length(indices)]

# ╔═╡ 0230aa6f-6f62-4314-beab-6838645fe6f7
function draw(grid)
	grid_box = Hyperrectangle(low=grid.lower_bounds,
                          high=grid.lower_bounds .+ grid.step .* [size(grid.acts)...])
	plot()
	plot!(grid_box, c=:white, aspectratio=:equal)
	for idx in CartesianIndices(grid.acts)
	    box, acts = get_cell(idx, grid)
	    c = acts == tt ? :gray : acts == tf ? :yellow : acts == ft ? :orange : :black
	    closed = true
	    plot!(box, c=c, alpha=1)
	end
	plot!()
end

# ╔═╡ c45d2def-560e-4162-a37a-8aa5aceab2f8
draw(grid)

# ╔═╡ 363a406d-7102-49a5-a2c3-91e795b0ada4
md"""
### Dynamics
"""

# ╔═╡ 9fd94ca3-f415-440a-9377-6072928aea61
A1 = [ 0 1;
      -1 0.]

# ╔═╡ c5c405a5-74f3-44f7-9fde-fcdec132a1a8
A2 = [-1  1;
      -1 -1.]

# ╔═╡ 0ba11b13-80ec-4bd9-bff7-901dd36e4571
As = Dict(1 => A1, 2 => A2)

# ╔═╡ dbef4735-e877-4522-9653-f143a7d7640d
period = 0.3

# ╔═╡ 91cbf043-f3bb-44e9-ba93-df435b3a90df
Φs = successor_preprocess(As, period)

# ╔═╡ 88d3bd32-c5b3-4a35-b8a6-00705b0c5e54
...
# ╔═╡ d0955950-e9c7-4bd9-b7f5-465d9a9ad887
md"""
`x1 =` $(@bind x1 NumberField(-0.1:0.001:0.1, default=0))
`x2 =` $(@bind x2 NumberField(-0.1:0.001:0.1, default=0))
"""

# ╔═╡ 13837e88-68ba-4429-a5c7-b87cc23df27e
function euler_approx(M, X, period; step=0.001)
	for i in 0:step:period
		X += step*M*X
	end
	X
end

# ╔═╡ 533b4f7b-2625-4173-a457-61bd413c29ea
euler_approx(A1, [x1, x2], period)

# ╔═╡ a505a0a0-db6f-44a9-855d-14cc8a15c197
euler_approx(A2, [x1, x2], period)

# ╔═╡ 8482c4e9-f3b3-4bd5-b626-636153f76311
let
	draw(grid)
	...
	x1′, x2′ = euler_approx(A1, [x1, x2], period)
	
	scatter!([x1′],[x2′], 
		label="A1", 
		marker=(:circle, :green, 4))
	
	x1′, x2′ = euler_approx(A2, [x1, x2], period)
	
	scatter!([x1′],[x2′], 
		label="A2", 
		marker=(:circle, :blue, 4))
end

# ╔═╡ 4eb3ad63-9935-4d3f-b848-f132a3814591
md"""
### Closure

low alpha if successors leave grid
"""

# ╔═╡ 19e53889-03c7-4017-b5d8-a04d22dad553
function draw_closure(grid, Φs)
	grid_box = Hyperrectangle(low=grid.lower_bounds,
                          high=grid.lower_bounds .+ grid.step .* [size(grid.acts)...])
	plot()
	...
	for idx in CartesianIndices(grid.acts)
	    ...
	    alpha = closed ? 1 : 0.2  # low alpha if successors leave grid
	    plot!(box, c=c, alpha=alpha)
	end
	plot!(aspectratio=:equal)
end

# ╔═╡ 7d84d8cf-c9ec-41cd-b39a-28d378e2feed
draw_closure(grid, Φs)

# ╔═╡ 26090cae-a0fc-4054-aa3d-af768833824d
md"""
## plot the successors
"""

# ╔═╡ 4d039e07-cb6b-427b-bc95-158f6285f9dd
md"""
**pick one of the following cases (1-4)**

`case =` $(@bind case Select([1, 2, 3, 4]))
"""

# ╔═╡ 28fdf14d-9431-4760-b8e3-c3abe076f711
if case == 1
...
elseif case == 4
    ### 4. case: One that I (Asger) got confused about
    cell = CartesianIndex(2, 1)
else
    error("undefined case")
end

# ╔═╡ 7db88327-ee8a-40f7-94bd-edf0c7de55a5
cell

# ╔═╡ 58fc878f-8adc-4747-a76c-3779ffa1f8ac
box, acts = get_cell(cell, grid)

# ╔═╡ e471b6c2-78cd-4622-bd9a-a259f002fefd
begin
	draw_closure(grid, Φs)
	plot!(box, fillalpha=0, line=(8, :blue))
end

# ╔═╡ 6387f3d3-5f93-4349-be35-516c11b3e566
begin
	succs = Zonotope[]
	for (i, act_i) in enumerate(acts)
	    if !act_i  # skip deactivated actions
...
# ╔═╡ dad190ae-33a1-4e61-a2d3-822ffe8eeb45
md"""
...
counterexample is S2
"""

# ╔═╡ 6e2966fc-3d45-44ed-8745-31d6de5d52b2
S1 = succs[1]

# ╔═╡ 0865462c-434a-4b99-a304-38e9b4de7364
check_cells_approximate(S1, grid)

# ╔═╡ 38a4ec1c-5003-4e2b-a958-d2f5cf48e9d7
check_cells_exact(S1, grid)

# ╔═╡ 80ddb292-1d69-4292-8df6-d17b5f2fe071
begin
	draw_closure(grid, Φs)
	plot!(box, fillalpha=0, line=(8, :blue))
	plot!(box_approximation(S1), c=:red)
	plot!(S1, c=:red, alpha=0.4, label="successor 1")
end

# ╔═╡ 299ee1cd-a603-43b2-b444-2f2ac0e7931c
S2 = succs[2]

# ╔═╡ 3c6b4201-1049-434f-87cb-b30e715153ea
check_cells_approximate(S2, grid)

# ╔═╡ f5a0752f-0e6f-45fd-a2b0-1684bbf88562
check_cells_exact(S2, grid)

# ╔═╡ 7fd2c204-7d0f-4348-9645-59ab9d1198bf
begin
	draw_closure(grid, Φs)
	plot!(box, fillalpha=0, line=(8, :blue))
	plot!(box_approximation(S2), c=:red)
	plot!(S2, c=:red, alpha=0.4, label="successor 2")
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "7d53d09daec21d2f813d15aadaa0afc5042a2767"
...
git-tree-sha1 = "8ef4379a054a0a2f4c5a13cfcc8e72e89d4ed7c3"
