...
function units(number_of_strategies)
	...
	for i in 1:(number_of_strategies)
		result["%unit $i%"] = "PreTrainedProductionUnit"
	end
	result["%unit $(number_of_strategies + 1)%"] = "ControllableProductionUnit"
	for i in (number_of_strategies + 2):N_UNITS
...
#=╠═╡
create_fleet(blueprint_path, strategy_paths, shield_path, output_dir)
  ╠═╡ =#
